...
function move(dom,json,callback){       	  //运动封装最终版（可以同时使元素多个属性同时变化的运动函数）      
  ...
  // function ajaxGet(url,callback){          //ajax封装
  ...
  //   }
  // }
  // ajaxGet( 'http://localhost/1814/0919/index.php',function(res){
  //   console.log(res);
  // });


   function ajaxGet(url){           //ajax完整版封装 （promise解决回调函数问题）
...
function ajaxPost(url,data){
    ...
    function jsonp(url,cb){          //jsonp实现跨域函数封装
        return new Promise(function(succ){
            // 提取出了前端的全局函数; 可以任性随便写;
